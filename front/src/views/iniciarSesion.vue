      ...
      <div class="row">
        <div class="col-6 my-auto">
          ...
          <div class="container-md">
            <p class="text-center purple--text text-h5">
              Tiene como objetivo ofrecer una solución eficiente para la organización de tareas y la planificación de clases en el entorno académico.
            </p>
          </div>
        </div>
        <div class="col-6 my-auto">
  ...
  methods: {
    async login() {
      ...
      if (this.usuario !== '' && this.contrasena !== '') {
          ...
          try {
            let response = await axios.post('http://localhost:3001/auth', autentication);
            if (response.status === 200) {
              ...
              if (response.data.rol === 'Docente') {
                ...
                for (let i in response.data.asignatures) {
                  this.array.push(response.data.asignatures[i].name);
                }
                this.$store.commit('setAsignaturasDocente', this.array);
