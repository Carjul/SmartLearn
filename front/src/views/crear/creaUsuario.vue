        ...
        <v-row>
          ...
          <v-col col="8" xs="4">
            ...
            <div>
                ...
                <v-stepper v-model="e1" style="margin:10px;">
                  ...
                  <v-stepper-items class="fondocard">
                    <v-stepper-content step="1">
                      <v-card class="mb-12 fondocaja" height="auto">
                        <v-row>
                          <v-col cols="12" sm="6">
                            <v-text-field background-color="#D4D4D4" style="margin:10px;font-weight: bold;" v-model="menu"
                              label="Rol" readonly outlined dense color="blue" autocomplete="false" class="mt-4" />
                          </v-col>

                          <v-col cols="12" sm="6">
                            <v-text-field style="margin:10px" v-model="name" background-color="#D4D4D4"
                              onkeyup="javascript:this.value=this.value.toUpperCase();" label="Digite Nombres" outlined
                              dense color="blue" autocomplete="false" class="mt-4" />
                          </v-col></v-row>
                        <v-row style="margin-top:-60px">
                          <v-col cols="12" sm="6">
                            <v-text-field style="margin:10px" v-model="apellido" background-color="#D4D4D4"
                              onkeyup="javascript:this.value=this.value.toUpperCase();" label="Digite Apellidos" outlined
                              dense color="blue" autocomplete="false" class="mt-4" />
                          </v-col>

                          <v-col cols="12" sm="6">

                            <v-select style="margin:10px" v-model="selected" background-color="#D4D4D4" label="Sexo" dense
                              outlined :items="items">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row style="margin-top:-60px">
                          <v-col cols="12" sm="6">
                            ...
                            <vs-input v-model="usuario"
                              style=" font-weight: bold;background-color:#D4D4D4;margin:10px;margin-top:30px; color:#636060;border:solid 1px;border-radius:5px"
                              border label-placeholder="Usuario O Email">
                          ...
                          <v-col cols="12" sm="6">
                            <v-text-field style="margin:10px;margin-top:30px" v-model="contrasena1"
                              background-color="#D4D4D4" type="number" label="N° Identificacion" outlined dense
                          ...
                          <v-row style="margin-top:-60px">
                            ...
                            <v-col cols="12" sm="6">
                              <v-text-field v-if="menu === 'Estudiante'" style="margin:10px"
                                v-model="acudientes.direccion" background-color="#D4D4D4"
                                onkeyup="javascript:this.value=this.value.toUpperCase();" label="Direccion Acudiente"
                      ...
                      <v-btn
                        v-if="name != '' && apellido != '' && selected != '' && menu == 'Administrador' && usuario != '' && contrasena1 != ''"
                        color="green" @click="crearDocente()">
                        Guardar
                      </v-btn>
                      <template v-if="menu == 'Administrador'">
                        <v-btn v-if="name == '' || apellido == '' || selected == '' || usuario == '' || contrasena1 == ''"
                    ...
                    <v-stepper-content step="2" v-if="menu == 'Docente'">
                      ...
                      <v-card class="mb-12 fondocaja">
                        <v-row>
                          <v-col cols="12" sm="6">
                            <v-text-field style="margin:10px" v-model="titulo" background-color="#D4D4D4"
                              onkeyup="javascript:this.value=this.value.toUpperCase();" label="Titulo Del Docente"
                          ...
                          <v-col cols="12" sm="6">
                            ...
                            <div class="input-group mb-2" style="margin:10px;margin-left:-10px">
                              <v-select v-model="asignatu" background-color="#D4D4D4" style=" width:60%;"
                                :items="asignaturas" item-text="name" label="Asignaturas" dense outlined></v-select>
                              <button class="input-group-append" @click="buscarAsignatura()">
                                <span style="height: 60%; color:green;background-color:none"><b-icon icon="gear-fill"
                                    font-scale="2"></b-icon></span>
                              </button>
                            </div>
                          </v-col>
                        </v-row>
                        ...
                        <v-row style="margin-top:-50px">
                          <v-col cols="12" sm="6">
                            <v-select v-model="grados_asigantira" background-color="#D4D4D4"
                              @input="buscarGrado_asignatura(grados_asigantira)" style="margin:10px"
                              :items="grados_asigantura" item-text="" label="Grados2" dense outlined></v-select>
                          </v-col>
                          <v-col cols="12" sm="6">
                            <div class="input-group mb-2" style="margin:10px;margin-left:-10px">
                              <v-select v-model="curso" background-color="#D4D4D4" style=" width:80%"
                                :items="asignarcurso_asigantura" return-object item-text="nomenclature" filled chips
                                label="cursos" multiple dense outlined></v-select>



                              <button v-if="gradosAsignados.length == 0" class="input-group-append"
                                @click="guardarAsignatura()">
                                <span style="height: 60%; color:green;background-color:none;"><b-icon icon="gear"
                                    font-scale="2"></b-icon></span>
                              </button>
                            </div>
                          </v-col>
                        ...
                        <v-col cols="12" sm="12">
                          <div style="border:solid #9B9494 0px;margin-top:-30px">
                            ...
                            <label v-for="(roles, index) in asignaturasAsignadas" :key="index" style="margin-left:10px">

                              <label style="font-size:15px">{{ roles.name }}
                                <label v-for="(cur, i) in roles.curso" :key="i"> <b-icon style="color:green;"
                                    icon="arrow-right" font-scale="1"></b-icon> {{ cur.nomenclature }}</label>
                              </label>

                              <b-icon @click="aliminarAsignatura(index)" style="color:red; height: 20px;margin-top:-20px;"
                                icon="x-circle-fill" font-scale="1"></b-icon>
                            </label>
                          </div>
                        </v-col>
                      </v-card>

                      <v-btn style=" margin-right:10px" color="rgba(0, 0, 255, 0.356)" @click="e1 = 1"
                      ...
                      <v-btn v-if="titulo != '' && asignaturasAsignadas.length >= 1" color="green"
                        @click="crearDocente()">
                        Guardar
                      </v-btn>

                      <v-btn disabled v-if="titulo == '' || asignaturasAsignadas.length <= 0">
                        Guardar
                      </v-btn>
                    ...
                    <v-stepper-content step="2" v-if="menu == 'Estudiante'">
                      <v-card class="mb-12 fondocaja" height="auto">
                        <v-row>
                          <v-col cols="12" sm="6">
                            <v-select v-model="grados" background-color="#D4D4D4" @input="buscarGrado(grados)"
                              style=" width:80%;margin:10px" :items="gradoscolegio" item-text="" label="Grados2" dense
                              outlined></v-select>
                          </v-col>
                          <v-col cols="12" sm="6">
                            <div class="input-group mb-2" style="margin:10px;margin-left:-20px">
                              ...
                              <v-select v-model="curso" background-color="#D4D4D4" style=" width:80%"
                                :items="asiganrCurso" item-text="nomenclature" label="cursos" dense outlined></v-select>


                              <button v-if="gradosAsignados.length == 0" class="input-group-append"
                                @click="guardarGrado()">
                                <span style="height: 60%; color:green;"><b-icon icon="gear"
                                    font-scale="2"></b-icon></span>
                              </button>
                            </div>
                          </v-col>
                        ...
                        <v-col cols="12" sm="12">
                          <div style=" text-align: center;">
                            ...
                            <label v-for="(roles, index) in gradosAsignados" :key="index" style="margin-left:10px">
                              <v-avatar size="60">
                                <label style="font-size:20px">{{ roles.nomenclature }}</label>
                              </v-avatar>
                              <b-icon @click="aliminarGrado(index)" style="color:red; height: 20px;margin-top:-20px;"
                                icon="x-circle-fill" font-scale="1"></b-icon>
                            </label>
                          </div>
                        </v-col>
                      ...
                      <v-btn v-if="gradosAsignados.length >= 1" color="green" @click="crearDocente()">
                        Guardar
                      </v-btn>

                      <v-btn disabled v-if="gradosAsignados.length <= 0">
                        Guardar
                      </v-btn>
              ...
              <v-snackbar v-model="hasSaved" :timeout="2000" absolute bottom left color="green">
                {{ msg }}
              </v-snackbar>
    ...
    guardarAsignatura() {
      for (let i = 0; i < this.asignaturas.length; i++) {
        if (this.asignaturas[i].name == this.asignatu) {
          ...
          this.as_asigantira = null;
          //  this.curso=nusignatu=null;
          this.gradoll;
      ...
      axios.post('http://localhost:3001/peoples', json)
        ...
        .catch((error) => {
          if (error.response.status === 500) {
            this.msg = "Ocurrió Un Error. Vuelve A Intentar";
            this.hasSaved = true;
          }
        });
