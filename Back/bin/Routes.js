...
app.post("/noticias/docente", function(req, res) {
    ...
    // Verifica si el docente tiene permiso para crear noticias
    if (docente && docente.role === "docente") {
        // El docente está autenticado y tiene permiso
        // Luego, puedes llamar a tu función para crear una noticia
        Controller.createNotice(news, res);
    } else {
        // El docente no está autenticado o no tiene permiso
        res.status(403).json({ message: "No tienes permiso para crear noticias." });
    }
});
